import { Globe, Target, Smartphone, ShieldCheck, BatteryLow } from "lucide-react";
        ...
        <div className="grid grid-cols-1 gap-8 sm:gap-10 lg:gap-16 lg:grid-cols-2 items-center mb-12 sm:mb-16 lg:mb-20">
          <ScrollReveal direction="left">
            <div className="relative rounded-2xl sm:rounded-3xl overflow-hidden shadow-elevated">
              <img src={communityImage} alt="South African youth community recycling team" className="w-full h-64 sm:h-80 lg:h-96 object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-foreground/40 to-transparent" />
              <div className="absolute bottom-4 left-4 right-4 sm:bottom-6 sm:left-6 sm:right-6">
                <p className="text-sm sm:text-base text-primary-foreground font-medium">🇿🇦 Built in South Africa, for South Africa</p>
              </div>
            </div>
          </ScrollReveal>
            ...
            <div>
              ...
              <div className="rounded-xl sm:rounded-2xl bg-muted p-4 sm:p-6 border border-border">
                ...
                <ul className="space-y-2 text-sm sm:text-base text-muted-foreground">
                  <li className="flex items-start gap-2"><span className="text-primary mt-1">✓</span>Enable unemployed youth to become recycling collectors</li>
                  <li className="flex items-start gap-2"><span className="text-primary mt-1">✓</span>Help collectors build and manage a trusted client base</li>
                  <li className="flex items-start gap-2"><span className="text-primary mt-1">✓</span>Track recyclable items and weekly collections</li>
                  <li className="flex items-start gap-2"><span className="text-primary mt-1">✓</span>Estimate weekly earnings for collectors</li>
                  <li className="flex items-start gap-2"><span className="text-primary mt-1">✓</span>Keep all interactions safe and inside the app</li>
                </ul>
              </div>
            </div>
          </ScrollReveal>
        ...
        <ScrollReveal>
          <div className="mx-auto max-w-4xl">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
              {[
                { icon: ShieldCheck, label: "No WhatsApp or off-platform chat" },
                { icon: ShieldCheck, label: "No phone numbers shared" },
                { icon: ShieldCheck, label: "No in-app payments or cash" },
                { icon: Globe, label: "No maps or GPS needed" },
                { icon: Smartphone, label: "Mobile-first, Android-first" },
                { icon: BatteryLow, label: "Low data usage" },
              ].map((item, i) => (
                <ScrollReveal key={i} delay={i * 0.08}>
                  <ConstraintCard icon={item.icon} label={item.label} />
                </ScrollReveal>
              ))}
            </div>
          </div>
